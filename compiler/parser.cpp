	...
	while (!parsing.empty())
	{
		...
		if (isTerminal(parsing.top()))//栈顶是终结符，和剩余代码比对，匹配则都出栈，否则进入错误处理
		{
			...
			else//不匹配，开始错误处理
			{
				...
				while (tmp != syntaxTree)//一直往上递归找follow集，直到遇到; end或者.
				{
					...
					for (int i = tmp->getWz() + 1; i < tmp->getFather()->child.size(); ++i)
					{
						...
						if (tmp->getFather()->child[i]->getType() == GrammarSymSpace::SEMICOLON ||
							...
							tmp->getFather()->child[i]->getType() == GrammarSymSpace::CONSTSYM ||
				...
				while (1)
				{
					...
					if (lexerTypeToGrammarType(currentToekn) == GrammarSymSpace::CONSTSYM ||
						...
						lexerTypeToGrammarType(currentToekn) == GrammarSymSpace::REPEAT)
					{
						...
						while (
							grammar.first.find(parsing.top())->second.find(GrammarSymSpace::CONSTSYM) == grammar.first.find(parsing.top())->second.end() &&
						...
						while (1)
						{
							...
							{
								if (grammar.first.find(currentNode->getFather()->child[i]->getType())->second.find(GrammarSymSpace::CONSTSYM) != grammar.first.find(currentNode->getFather()->child[i]->getType())->second.end() ||
		...
		else//栈顶为非终结符，查预测表，有相应的项目则用产生式替换，否则进入错误处理
		{
			...
			else//预测表中找到了对应的项目，用产生式替换。
			{
				...
				pp.v = grammar.parsingTable.find(parsingNode(top, lexerTypeToGrammarType(currentToekn)))->second;
...
void Parser::handleConditionStatementPlus(AstNode * n, sTable * s)
{
	...
	if (currentNode->child[0]->getType() == GrammarSymSpace::ELSE)
	{
		...
		for (int i = pcode.size() - 1; i > -1; --i)
		{
			if (pcode[i].op == "JMP" && pcode[i].a2 == 0x3f3f3f)
			{
				pcode[i].a2 = pcode.size();
				break;
			}
...
list<list<vector<int>>>::iterator Parser::getGrammarList()
{
	return grammar.getGrammarList();
}

list<list<vector<int>>>::iterator Parser::getGrammarListEnd()
{
	return grammar.getGrammarListEnd();
}

map<int, set<int>>::iterator Parser::getFirstBegin()
{
	return grammar.first.begin();
}

map<int, set<int>>::iterator Parser::getFirstEnd()
{
	return grammar.first.end();
}

map<int, set<int>>::iterator Parser::getFollowBegin()
{
	return grammar.follow.begin();
}

map<int, set<int>>::iterator Parser::getFollowEnd()
{
	return grammar.follow.end();
}

map<parsingNode, vector<int>>::iterator Parser::getParsingTableBegin()
{
	return grammar.parsingTable.begin();
}

map<parsingNode, vector<int>>::iterator Parser::getParsingTableEnd()
{
	return grammar.parsingTable.end();
}

vector<pCode>::iterator Parser::getPCodeBegin()
{
	return pcode.begin();
}

vector<pCode>::iterator Parser::getPCodeEnd()
{
	return pcode.end();
}
